...
for n in range(len(hXX_list)):
    ...
    for s in range(len(simID)):
        ...
        for p in range(len(pop_list)):
            ...
            Pheno_path = file_path_roh + str(simID[s]) + '/pheno_neutral_recalculation/gravel.del.only.'+str(hXX_list[n])+'.'+str(simID[s])+ '.trees.'+str(pop_list[p])+'.exon.in.smapled.neutral.mutation.rho.0.0.tau.1.0.pheno.normalized.txt'
            ...
            # Start from dealting with ROH file
...
final_summary_df.to_csv(
    'calculate_0_proportion_neutral.csv', 
    index=False,   
    sep=',',       
    encoding='utf-8' 
)