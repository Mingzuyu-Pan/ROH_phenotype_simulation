...
rhostr = ["0.0"]
taustr = ["0.7","0.8","0.9","1.0","1.1","1.2","1.3"]
for n in range(len(hXX_list)):
    ...
    for s in range(len(simID)):        
        for j in range(len(rhostr)):
            for k in range(len(taustr)):
                file_path_data = './'+str(hXX_list[n])+'/' + str(simID[s]) + '/pheno_neutral_recalculation'
                p1_path = file_path_data + '/gravel.del.only.'+str(hXX_list[n])+'.'+str(simID[s])+'.trees.p1.exon.in.smapled.neutral.mutation.rho.'+str(rhostr[j])+'.tau.'+str(taustr[k])+'.pheno.normalized.txt'
                p2_path = file_path_data + '/gravel.del.only.'+str(hXX_list[n])+'.'+str(simID[s])+'.trees.p2.exon.in.smapled.neutral.mutation.rho.'+str(rhostr[j])+'.tau.'+str(taustr[k])+'.pheno.normalized.txt'
                p3_path = file_path_data + '/gravel.del.only.'+str(hXX_list[n])+'.'+str(simID[s])+'.trees.p3.exon.in.smapled.neutral.mutation.rho.'+str(rhostr[j])+'.tau.'+str(taustr[k])+'.pheno.normalized.txt'
    ...
    filename = f"{hXX_list[n]}_summary_proportion_neutral_normalized.xlsx"
