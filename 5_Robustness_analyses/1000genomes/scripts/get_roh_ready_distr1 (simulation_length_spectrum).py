...
for hXX in hXX_list:
    ...
    pop_list = ['p1','p2','p3']
    out_dir = f'{hXX}_simulated_ROH_Summary.parquet_dataset'
    ...
    for simID in sim_list:
        for pop in pop_list:
            ...
            out_file = os.path.join(out_dir, f'{hXX}_sim_{simID}_pop_{pop}.parquet')
            df_python.to_parquet(out_file, engine='pyarrow', index=False)