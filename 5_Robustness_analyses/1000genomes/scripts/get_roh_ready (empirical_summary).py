...
for pop in pop_list:
    ...
    summary_chr1 = df_chr1.groupby(['ind', 'pop','superpop', 'class']).agg(
        nROH=('ind', 'size'),           
        sROH_cM=('cM_length', 'sum'), 
        sROH_bp=('bp_length', 'sum')  
    ).reset_index()
    chr1_summary_dfs.append(summary_chr1)
    
    df_chr1_100M = df_chr1[df_chr1['end'] <= 100000000] 
    summary_chr1_100M = df_chr1_100M.groupby(['ind', 'pop','superpop', 'class']).agg(
        nROH=('ind', 'size'),           
        sROH_cM=('cM_length', 'sum'), 
        sROH_bp=('bp_length', 'sum')  
    ).reset_index()
    chr1_100M_summary_dfs.append(summary_chr1_100M)
    
    summary_df = df_python.groupby(['ind', 'pop','superpop', 'class']).agg(
        nROH=('ind', 'size'),           
        sROH_cM=('cM_length', 'sum'), 
        sROH_bp=('bp_length', 'sum')  
    ).reset_index()
...
final_master_chr1 = pd.concat(chr1_summary_dfs, ignore_index=True)        
final_master_chr1.to_csv('All_Populations_chr1_ROH_Summary.csv', index=False)
final_master_chr1_100M = pd.concat(chr1_100M_summary_dfs, ignore_index=True)        
final_master_chr1_100M.to_csv('All_Populations_chr1_100MB_ROH_Summary.csv', index=False)
final_master_df = pd.concat(all_summary_dfs, ignore_index=True)        
final_master_df.to_csv('All_Populations_ROH_Summary.csv', index=False)
