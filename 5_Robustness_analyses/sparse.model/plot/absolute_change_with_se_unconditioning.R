...
df_ready = df_diff_filtered %>% group_by(hXX,pop,tau,Region_type) %>%
  summarise(mean_diff_log_all = mean(diff_log_all),
            se_diff_log_all = sd(diff_log_all)/sqrt(500),
            .groups = 'drop')

p_test = ggplot(df_ready, aes(x = tau, y = mean_diff_log_all, color = hXX, linetype = pop)) +
  geom_errorbar(aes(ymin = mean_diff_log_all - se_diff_log_all, ymax =  mean_diff_log_all + se_diff_log_all, color = hXX), width = 0.04, alpha = 0.8,linetype = "solid") +
  ...
  labs(
    ...
    x = "Tau",  
    y = "Difference in per cM contribution between ROH and Non-ROH Regions (Unconditional)",  
  )

print(p_test)
ggsave(
  filename = paste0('change_per_cM_unconditional.x.',x,'.pdf'),
  plot     = p_test,
  device   = cairo_pdf,
  width    = 6.5,
  height   = 8,
