...
df_raw = read_csv(paste0('h00_cM_Mb_combined.csv'))
df_long = df_raw %>%
  pivot_longer(
    cols = -c(pop, tau, hXX, simID, unit),
...
for (i in 1:nrow(panel_config)) {
  ...
  y_title = if(cfg$show_y_title) "Conditional genotypic score per unit (ln)" else NULL
...
ggsave(
  filename = paste0('cM_Mb_compare.x.1.0.compare.pdf'),
...
all_stat = df_long %>%
  group_by(pop, tau, unit) %>%
  arrange(simID, .by_group = TRUE) %>%
  wilcox_test(per_cM_nonzero_log ~ Region_type, paired = TRUE, p.adjust.method = "holm") %>%
  add_significance(p.col = "p.adj", cutpoints = c(0, 0.001, 0.01, 0.05, 1), 
                   symbols = c("***", "**", "*", "ns"))

write.csv(all_stat, "cM_Mb_comparison_stat_results.csv", row.names = FALSE)
